...
class RenderManager:
    ...
    def cleanup(self):
        ...
        camera_objects = [obj for obj in bpy.data.objects 
                         if obj.type == 'CAMERA' and 
                         (obj.name.startswith('Render_Camera') or obj.name.startswith('Top_Down_Camera'))]
        
        camera_data = [cam for cam in bpy.data.cameras 
        ...
        if hasattr(self, 'original_lights'):
            light_objects = [obj for obj in self.context.scene.objects 
                            if obj.type == 'LIGHT' and 
                            obj.name.startswith(('Key_Light', 'Fill_Light', 'Back_Light', 'Sun_Light'))]
